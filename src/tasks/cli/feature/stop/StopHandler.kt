...
class StopHandler(private val taskStore: Memoized<out ObservableTaskStore>) : ArgumentHandler<StopArguments> {

  override fun handle(arguments: CommonArguments<out StopArguments>): Single<Output> {
    // Validate arguments
    ...
    return Observable.fromIterable(arguments.specificArguments().tasks())
        ...
        .reduce(
            Tuplet.of<ImmutableSet.Builder<TaskId>>(
                ImmutableSet.builder(),   // tasks that could not be stopped
                ImmutableSet.builder()),  // tasks that were stopped
            { builders, mutationResult ->
              val becameStopped = (mutationResult.third().status().isOpen
                  && mutationResult.first()
                  .lookUpById(mutationResult.third().id())
                  .map { !it.status().isOpen }
                  .orElse(false))
              (if (becameStopped) builders.second() else builders.first())
