...
import tasks.util.rx.Observables.incrementingLong
import tasks.util.rx.Observables.toImmutableMap
import tasks.util.rx.Observables.toImmutableSet
...
class ObservablesTest {
  ...
  fun incrementingInteger_multipleSubscribers_doesNotShareState() {
    ...
    val underTest = incrementingInteger().take(expected.size.toLong())
  ...
  fun incrementingInteger_withIncrement_increments() {
    ...
    incrementingInteger(0, 7).take(expected.size.toLong()).test().assertValues(*expected)
  }
  ...
  fun incrementingInteger_withIncrement0_doesNotIncrement() {
    ...
    incrementingInteger(0, 0).take(expected.size.toLong()).test().assertValues(*expected)
  }
  ...
  fun incrementingInteger_withIncrementNegative_decrements() {
    ...
    incrementingInteger(0, -1).take(expected.size.toLong()).test().assertValues(*expected)
  }
  ...
  fun incrementingInteger_withStart_startsAtStart() {
    ...
    incrementingInteger(93, 1).take(expected.size.toLong()).test().assertValues(*expected)
  }
  ...
  fun incrementingLong_multipleSubscribers_doesNotShareState() {
    ...
    val underTest = incrementingLong().take(expected.size.toLong())
    underTest.test().assertValues(*expected)
    underTest.test().assertValues(*expected)
  }
  ...
  fun incrementingLong_withIncrement_increments() {
    ...
    incrementingLong(0, 7).take(expected.size.toLong()).test().assertValues(*expected)
  }
  ...
  fun incrementingLong_withIncrement0_doesNotIncrement() {
    ...
    incrementingLong(0, 0).take(expected.size.toLong()).test().assertValues(*expected)
  }
  ...
  fun incrementingLong_withIncrementNegative_decrements() {
    ...
    incrementingLong(0, -1).take(expected.size.toLong()).test().assertValues(*expected)
  }
  ...
  fun incrementingLong_withStart_startsAtStart() {
    ...
    incrementingLong(93, 1).take(expected.size.toLong()).test().assertValues(*expected)
  }
  ...
  fun toImmutableMap_whenHot_withLateSubscribers_doesNotShareState() {
    val source: Subject<Int> = PublishSubject.create()
    val underTest = source.to(toImmutableMap<Int, Int, Int>({ it }, { it }))
    val firstObserver = underTest.test()
    source.onNext(0)
    source.onNext(1)
    source.onNext(2)
  ...
  fun toImmutableSet_whenHot_withLateSubscribers_doesNotShareState() {
    val source: Subject<Int> = PublishSubject.create()
    val underTest = source.to(toImmutableSet())
    val firstObserver = underTest.test()
    source.onNext(0)
    source.onNext(1)
    source.onNext(2)
