...
class RemoveHandlerTest {
  ...
  private val underTest = RemoveHandler(
      just(taskStore),
      Printer.Factory { PrintStream(outputStream) },
      just(inputReader))

  @Test
  fun handle_withNoTasks_throwsException() {
    val exception = Assertions.assertThrows(HandlerException::class.java) { underTest.handle(removeArgsForcefully()) }
    Truth.assertThat(exception).hasMessageThat().isEqualTo("no tasks specified")
  }
  ...
  fun handle_whenForced_withOneTask_removesTask_printsRemovedTask() {
    ...
    HandlerTestUtils.assertOutputContainsGroupedTasks(output, TASKS_DELETED_HEADER, ImmutableList.of(task))
  ...
  fun handle_whenPrompted_withOneTask_printsConfirmationPrompt_doesNotDeleteTaskYet() {
    ...
    Truth.assertThat(outputStream.toString()).containsMatch(
        Pattern.compile(
    ...
    Truth.assertThat(outputStream.toString()).containsMatch(
        Pattern.compile(
            Pattern.quote(task1.render().renderWithoutCodes()) +
                ...
                Pattern.quote(DELETE_THIS_TASK_PROMPT),
            Pattern.MULTILINE))
    Truth.assertThat(outputStream.toString()).doesNotContain(task2.render().renderWithoutCodes())
    Truth.assertThat(outputStream.toString()).doesNotContain(task3.render().renderWithoutCodes())
  ...
  fun handle_whenPrompted_withThreeTasks_thenConfirmsAll_deletesTasks_printsDeletedTasks() {
    ...
    val task2 = createTask("example task 2")
    val task3 = createTask("example task 3")
    val subscription = underTest.handle(removeArgsWithPrompt(task1, task2, task3)).test().assertNotComplete()
  ...
  fun handle_whenPrompted_withOneTask_thenDeny_doesNotDeleteTask_outputsNothing() {
    ...
    Truth.assertThat(subscription.assertComplete().values()[0].renderWithoutCodes()).isEmpty()
    assertTasksNotDeleted(task)
  }
  ...
  fun handle_whenPrompted_withFourTasks_denyAndConfirm_behavesCorrectly() {
    ...
    HandlerTestUtils.assertOutputContainsGroupedTasks(output, TASKS_DELETED_HEADER, ImmutableList.of(task1, task3))
    Truth.assertThat(output).doesNotContain(task2.render().renderWithoutCodes())
    Truth.assertThat(output).doesNotContain(task4.render().renderWithoutCodes())
  ...
  fun handle_whenPrompted_withOneTask_unrecognizedInput_threeTimes_doesNotDeleteTask() {
    ...
    Truth.assertThat(intermediateOutput).matches(
        Pattern.compile(
            Pattern.quote(task.render().renderWithoutCodes()) +
                "\\s*" +
                Pattern.quote(DELETE_THIS_TASK_PROMPT) +
  ...
  private fun assertTasksNotDeleted(vararg tasks: Task) {
    assertThatEachTask({ obj: OptionalSubject -> obj.isPresent() }, *tasks)
  }

  private fun assertTasksDeleted(vararg tasks: Task) {
    assertThatEachTask({ obj: OptionalSubject -> obj.isEmpty() }, *tasks)
  }
  ...
  private fun assertThatEachTask(optionalAssertions: Consumer<OptionalSubject>, vararg tasks: Task) {
    ...
    for (task in tasks) {
      optionalAssertions.accept(Truth8.assertThat(taskStoreState.lookUpById(task.id())))
    }
  }
  ...
  companion object {
    ...
    private fun removeArgs(force: Boolean, vararg tasks: Task): CommonArguments<RemoveArguments> {
      return HandlerTestUtils.commonArgs<RemoveArguments>(RemoveArguments(copyOf(tasks), force))
    }
  }
}